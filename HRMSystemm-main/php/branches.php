...
function isAdmin(): bool {
    return !empty($_SESSION['user_id']) && trim((string)($_SESSION['user_role'] ?? '')) === 'Administrator';
}
...
$checkLatitude = $pdo->prepare("SELECT COUNT(*) FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'branches' AND COLUMN_NAME = 'latitude'");
$checkLatitude->execute();
if (!$checkLatitude->fetchColumn()) {
    $pdo->exec("ALTER TABLE branches ADD COLUMN latitude DECIMAL(10,7) NULL AFTER location");
}
$checkLongitude = $pdo->prepare("SELECT COUNT(*) FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'branches' AND COLUMN_NAME = 'longitude'");
$checkLongitude->execute();
if (!$checkLongitude->fetchColumn()) {
...
if ($action === 'list') {
    ...
    $baseQuery = "SELECT b.id, b.name, b.location, b.latitude, b.longitude, b.contact_details, b.status, b.created_at,
...
if ($action === 'public_list') {
    $stmt = $pdo->query("SELECT id, name, location, latitude, longitude, status, created_at FROM branches WHERE status = 'active' AND latitude IS NOT NULL AND longitude IS NOT NULL ORDER BY created_at DESC");
    branchRespond(true, '', ['branches' => $stmt->fetchAll()]);
}
...
if ($action === 'create' || $action === 'update') {
    ...
    $latitude = filter_var($_POST['branch_latitude'] ?? null, FILTER_VALIDATE_FLOAT);
    $longitude = filter_var($_POST['branch_longitude'] ?? null, FILTER_VALIDATE_FLOAT);
    ...
    if ($latitude === false || $longitude === false || $latitude < -90 || $latitude > 90 || $longitude < -180 || $longitude > 180) {
        ...
        branchRespond(false, 'Please select a valid branch location on the map.');
    }
    ...
    if ($action === 'create') {
        $stmt = $pdo->prepare('INSERT INTO branches (name, location, latitude, longitude, contact_details, required_baristas, required_cashiers, required_managers, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)');
        $stmt->execute([$name, $location, $latitude, $longitude, $contact, $requiredBaristas, $requiredCashiers, $requiredManagers, $status]);
    ...
    $stmt = $pdo->prepare('UPDATE branches SET name = ?, location = ?, latitude = ?, longitude = ?, contact_details = ?, required_baristas = ?, required_cashiers = ?, required_managers = ?, status = ? WHERE id = ?');
    $stmt->execute([$name, $location, $latitude, $longitude, $contact, $requiredBaristas, $requiredCashiers, $requiredManagers, $status, $id]);
...
if ($action === 'delete') {
    ...
    $exists = $pdo->prepare('SELECT id FROM branches WHERE id = ? LIMIT 1');
    $exists->execute([$id]);
    if (!$exists->fetch()) {
        http_response_code(404);
        branchRespond(false, 'Branch not found.');
    }

...
function openPosDatabaseConnection(): mysqli {
    $conn = new mysqli('localhost', 'root', '', 'quadra_cafe');
    if ($conn->connect_error) {
        throw new RuntimeException('Unable to connect to the POS database: ' . $conn->connect_error);
    }
    return $conn;
}

function ensurePosUserLinkColumns(mysqli $conn): void {
    $columns = [];
    $result = $conn->query('SHOW COLUMNS FROM users');
    while ($row = $result->fetch_assoc()) {
        $columns[] = $row['Field'];
    }
    if (!in_array('employee_id', $columns, true) && !$conn->query('ALTER TABLE users ADD COLUMN employee_id INT NULL AFTER id')) {
        throw new RuntimeException('Unable to add the employee_id linkage to the POS users table.');
    }
    if (!in_array('branch_id', $columns, true) && !$conn->query('ALTER TABLE users ADD COLUMN branch_id INT NULL AFTER employee_id')) {
        throw new RuntimeException('Unable to add the branch_id linkage to the POS users table.');
    }
}

function generatePosUserPin(mysqli $conn): string {
    do {
        $pin = str_pad((string)random_int(0, 999999), 6, '0', STR_PAD_LEFT);
        $check = $conn->prepare('SELECT id FROM users WHERE pin = ? LIMIT 1');
...
if ($action === 'final_confirmation' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        $complete = $pdo->prepare("UPDATE applicants SET status = 'hired', availability = 'not-available', pos_account_required = 1, pos_account_created_at = NULL WHERE id = ? AND status = 'final_confirmation_pending'");
        $complete->execute([$id]);
...
if (($action === 'create_pos_account' || $action === 'complete_pos_account') && $_SERVER['REQUEST_METHOD'] === 'POST') {
    requireRole('admin');
    $id = (int)($_POST['id'] ?? 0);
    if ($id < 1) respond(false, 'Invalid employee selection.');

    $stmt = $pdo->prepare("SELECT a.*, e.id AS hrms_employee_id, e.full_name AS employee_full_name, e.email AS employee_email, e.contact_no AS employee_contact_no, e.branch_id AS hrms_branch_id, e.position AS employee_position, e.status AS employee_status
        FROM applicants a
        LEFT JOIN employees e ON e.id = CAST(a.employee_id AS UNSIGNED)
        WHERE a.id = ? AND a.status = 'hired' AND a.pos_account_required = 1 LIMIT 1");
    $stmt->execute([$id]);
    $applicant = $stmt->fetch();
    if (!$applicant) respond(false, 'This employee is no longer eligible for a POS account.');

    $employeeId = (int)($applicant['hrms_employee_id'] ?? 0);
    if ($employeeId < 1) respond(false, 'The HRMS employee record for this hire is missing.');

    try {
        $posConn = openPosDatabaseConnection();
        ensurePosUserLinkColumns($posConn);

        $existingEmail = trim((string)($applicant['employee_email'] ?? $applicant['email'] ?? ''));
        $existing = $posConn->prepare('SELECT id, role, status FROM users WHERE employee_id = ? OR email = ? LIMIT 1');
        $existing->bind_param('is', $employeeId, $existingEmail);
        $existing->execute();
        $existingRow = $existing->get_result()->fetch_assoc();

        if ($existingRow) {
            $mark = $pdo->prepare("UPDATE applicants SET pos_account_created_at = NOW() WHERE id = ? AND pos_account_required = 1 AND pos_account_created_at IS NULL");
            $mark->execute([$id]);
            $posConn->close();
            respond(true, 'A matching POS account already exists for this employee. The HRMS task was marked as created.');
        }

        $role = stripos((string)($applicant['employee_position'] ?? ''), 'cashier') !== false ? 'Cashier' : 'Admin';
        $pin = generatePosUserPin($posConn);
        $branchId = isset($applicant['hrms_branch_id']) && $applicant['hrms_branch_id'] !== null ? (int)$applicant['hrms_branch_id'] : null;
        $branchName = 'Branch not assigned';
        if ($branchId) {
            $branchStmt = $pdo->prepare('SELECT name, location FROM branches WHERE id = ? LIMIT 1');
            $branchStmt->execute([$branchId]);
            $branchRow = $branchStmt->fetch();
            if ($branchRow) {
                $branchParts = array_filter([
                    trim((string)($branchRow['name'] ?? '')),
                    trim((string)($branchRow['location'] ?? ''))
                ]);
                $branchName = $branchParts ? implode(' — ', $branchParts) : 'Branch ID ' . $branchId;
            } else {
                $branchName = 'Branch ID ' . $branchId;
            }
        }
        $fullName = trim((string)($applicant['employee_full_name'] ?? trim(($applicant['first_name'] ?? '') . ' ' . ($applicant['last_name'] ?? ''))));
        $birthdate = trim((string)($applicant['birthdate'] ?? '')) ?: '2000-01-01';
        $address = trim((string)($applicant['address'] ?? '')) ?: 'N/A';
        $dateHired = !empty($applicant['created_at']) ? date('Y-m-d', strtotime($applicant['created_at'])) : date('Y-m-d');
        $email = trim((string)($applicant['employee_email'] ?? $applicant['email'] ?? ''));
        $cellphone = trim((string)($applicant['employee_contact_no'] ?? $applicant['contact_no'] ?? ''));

        $statusValue = 'Active';
        $insert = $posConn->prepare('INSERT INTO users (employee_id, branch_id, full_name, birthdate, address, date_hired, email, cellphone, pin, role, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)');
        $insert->bind_param('iisssssssss', $employeeId, $branchId, $fullName, $birthdate, $address, $dateHired, $email, $cellphone, $pin, $role, $statusValue);

        if (!$insert->execute()) {
            throw new RuntimeException('POS account insertion failed: ' . $insert->error);
        }

        $posUserId = (int)$posConn->insert_id;
        try {
            $subject = 'Your Quadra Cafe POS Login PIN';
            $body = "Dear {$fullName},\n\nYour POS account has been created successfully.\n\nEmployee ID: {$employeeId}\nPIN: {$pin}\nRole: {$role}\nBranch: {$branchName}\n\nUse this PIN to sign in to the CafePOS login page. Keep it secure and do not share it.\n\nRegards,\nQuadra Cafe HR Team";
            require_once 'send_email.php';
            sendQuadraEmail($email, $fullName, $subject, $body);
        } catch (Throwable $emailError) {
            $rollback = $posConn->prepare('DELETE FROM users WHERE id = ?');
            $rollback->bind_param('i', $posUserId);
            $rollback->execute();
            throw new RuntimeException('POS account creation failed during PIN email delivery and was rolled back: ' . $emailError->getMessage());
        }

        $mark = $pdo->prepare("UPDATE applicants SET pos_account_created_at = NOW() WHERE id = ? AND pos_account_required = 1 AND pos_account_created_at IS NULL");
        $mark->execute([$id]);
        if ($mark->rowCount() !== 1) {
            throw new RuntimeException('The HRMS task could not be marked as completed after the POS account was inserted and the PIN email was sent.');
        }

        $posConn->close();
        respond(true, 'POS account created successfully. The employee PIN has been emailed to the registered email address for privacy and security.');
    } catch (Throwable $error) {
        if (isset($posConn) && $posConn instanceof mysqli) {
            $posConn->close();
        }
        respond(false, 'POS account creation failed: ' . $error->getMessage());
    }
}

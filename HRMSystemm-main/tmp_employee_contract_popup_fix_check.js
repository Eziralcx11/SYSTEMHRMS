...
function renderEmployeeDocuments() {
  ...
  grid.innerHTML = visibleTypes.map(type => {
    ...
    const contractTitle = String(document.file_name || 'Employment Contract').replace(/\\/g, '\\\\').replace(/'/g, "\\'");
    const file = document
      ? '<button type="button" class="document-file-link" onclick="openContractPreview(' + Number(document.document_id) + ', \'' + contractTitle + '\')" style="border:0;background:none;padding:0;text-align:left;cursor:pointer;"><i class="fas fa-paperclip"></i> ' + escapeEmployeeDocumentHtml(document.file_name) + '</button>'
    ...
    return '<article class="government-document-item">' +
      ...
      (type.contract && document ? '<button type="button" class="btn btn-outline document-view-btn" onclick="openContractPreview(' + Number(document.document_id) + ', \'' + contractTitle + '\')"><i class="fas fa-eye"></i> View Contract</button>' : '') +
